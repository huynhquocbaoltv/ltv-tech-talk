...
[
  ...
  ...
          ...
          const endOffset = res.lastIndexOf("]") + 1;
          const result = JSON.parse(
            res.substring(startOffset, endOffset)
          ) as Result[];
          this._view?.webview.postMessage({ command: "setResult", result });
        } catch (error) {
          vscode.window.showErrorMessage(
            "An error occurred while analyzing code" + error
          );
        }
      }
    );
